...
def main():
    ...
    output_path = Path("/home/student/Toan/results/sweep_0_5_65_67.mp4")
    ...
    writer = cv2.VideoWriter(str(output_path), cv2.VideoWriter_fourcc(*'mp4v'), 
                            30, (w, h))
    print(f"Generating Sweep for x[0-5], y[65-67]: {output_path}")
    
    # Raster scan
    step = 0.1
    
    # Range inclusive
    for dy in np.arange(65, 67 + step, step):
        # Scan x
        xs = np.arange(0, 5 + step, step)
