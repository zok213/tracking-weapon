...
def main():
    ...
    output_path = Path("/home/student/Toan/results/sweep_11_12_62_64.mp4")
    ...
    writer = cv2.VideoWriter(str(output_path), cv2.VideoWriter_fourcc(*'mp4v'), 
                            30, (w, h))
    print(f"Generating Sweep for x[11-12], y[62-64]: {output_path}")
    ...
    for dy in np.arange(62, 64 + step, step):
        # Scan x
        xs = np.arange(11, 12 + step, step)
        if int(dy*10)%2 == 1: xs = xs[::-1] # Zigzag for smoothness
