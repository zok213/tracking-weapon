...
def train():
    ...
    dataset_yaml = project_root / "datasets/vtmot_rgbt/data.yaml"
    ...
    phase1_last = runs_dir / "vtmot_mcf_phase1/weights/last.pt"
    ...
    phase1_config = {
        ...
        'optimizer': 'AdamW', # Warm-up usually better with AdamW
        'lr0': 0.001,
        'warmup_epochs': 1,
        'freeze': freeze_layers,
        'project': str(runs_dir),
        'name': 'vtmot_mcf_phase1',
        'exist_ok': True,
        'cache': 'ram',
        'workers': 12,
    }
    
    print("\n--- PHASE 1: Warm-up (or Resuming) ---")
    trainer1 = MCFTrainer(overrides=phase1_config, mcf_model=model.model)
    trainer1.train()
    
    # Phase 2: Full Fine-Tuning
    best_phase1 = runs_dir / "vtmot_mcf_phase1/weights/best.pt"
    if not best_phase1.exists():
        best_phase1 = runs_dir / "vtmot_mcf_phase1/weights/last.pt"
    ...
    phase2_config = {
        ...
        'channels': 6,
        'pairs_rgb_ir': ['_rgb_', '_ir_'],
        'optimizer': 'SGD', # Standard research-grade fine-tuning
        'lr0': 0.0005,
        'lrf': 0.01,
        'cos_lr': True,
        'momentum': 0.937,
