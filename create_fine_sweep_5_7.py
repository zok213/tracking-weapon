...
def main():
    ...
    output_path = Path("/home/student/Toan/results/sweep_5_7_66_68.mp4")
    ...
    writer = cv2.VideoWriter(str(output_path), cv2.VideoWriter_fourcc(*'mp4v'), 
                            30, (w, h))
    print(f"Generating Sweep for x[5-7], y[66-68]: {output_path}")
    
    # Raster scan
    step = 0.1
    
    # Range inclusive
    for dy in np.arange(66, 68 + step, step):
        # Scan x
        xs = np.arange(5, 7 + step, step)
