#!/usr/bin/env python3
"""
Train Far View MCF Model v2.0 (YOLOv11x RGBT)
ENHANCED with AI Engineer recommendations:
- Stronger scale augmentation for small objects (0.9)
- Perspective/shear augmentation for drone footage
...
def train():
    ...
    print(f"Training FAR VIEW model v2.0 (ENHANCED - long-range person detection)")
    print(f"Dataset: {dataset_yaml}")
    print(f"Enhancements: early stopping, label smoothing, small-object augmentations")
    ...
    phase1_last = runs_dir / "far_view_mcf_phase1_v2/weights/last.pt"
    ...
    print("\n--- PHASE 1: Warm-up (Far View v2.0) ---")
    trainer1 = MCFTrainer(overrides=phase1_config, mcf_model=model.model)
    trainer1.train()
    
    # Phase 2: Full Fine-Tuning with ENHANCED augmentations
    best_phase1 = runs_dir / "far_view_mcf_phase1_v2/weights/best.pt"
    if not best_phase1.exists():
        best_phase1 = runs_dir / "far_view_mcf_phase1_v2/weights/last.pt"
    ...
    phase2_config = {
        ...
        'mixup': 0.15,
        'copy_paste': 0.1,
        'scale': 0.9,       # STRONGER scale augmentation for small objects
        'degrees': 5.0,     # NEW: Slight rotation for drone footage
        'shear': 2.0,       # NEW: Perspective variation
        'perspective': 0.001,  # NEW: For drone/UAV viewpoints
        'close_mosaic': 10,
        
        # === REGULARIZATION IMPROVEMENTS ===
        'label_smoothing': 0.1,  # NEW: Better calibration
        
        # === TRAINING IMPROVEMENTS ===
        'patience': 15,     # NEW: Early stopping
        'save_period': 5,   # NEW: Save every 5 epochs
        'plots': True,      # NEW: Generate training curves
        
        'freeze': None,
        'project': str(runs_dir),
        'name': 'far_view_mcf_phase2_v2',
        'exist_ok': True,
        'cache': 'ram',
        'workers': 12,
    }
    
    print("\n--- PHASE 2: Full Fine-tuning (Far View v2.0 ENHANCED) ---")
    print("Enhancements applied:")
    print("  - Scale augmentation: 0.9 (stronger for small objects)")
    print("  - Rotation: 5.0 degrees")
    print("  - Shear: 2.0")
    print("  - Perspective: 0.001")
    ...
    print("\n✓ Far View MCF v2.0 (ENHANCED) Training Complete!")
