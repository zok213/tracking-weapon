#!/usr/bin/env python3
"""
Train Near View MCF Model v3.0 (YOLOv11x RGBT) - MAXIMUM OPTIMIZATIONS
...
def train():
    ...
    print("=" * 70)
    print("NEAR VIEW MCF MODEL v3.0 - MAXIMUM OPTIMIZATIONS")
    print("=" * 70)
    ...
    phase1_last = runs_dir / "near_view_mcf_phase1_v3/weights/last.pt"
    ...
    print("\n--- PHASE 1: Warm-up (Near View v3.0) ---")
    trainer1 = MCFTrainer(overrides=phase1_config, mcf_model=model.model)
    trainer1.train()
    
    # Phase 2: Full Fine-Tuning
    best_phase1 = runs_dir / "near_view_mcf_phase1_v3/weights/best.pt"
    if not best_phase1.exists():
        best_phase1 = runs_dir / "near_view_mcf_phase1_v3/weights/last.pt"
    ...
    phase2_config = {
        ...
        # === AUGMENTATION (NEAR VIEW - MODERATE) ===
        'mosaic': 1.0,
        'mixup': 0.15,
        'copy_paste': 0.1,
        'scale': 0.7,          # Moderate scale (near view has larger objects)
        'translate': 0.1,
        'flipud': 0.0,
        'fliplr': 0.5,
        'close_mosaic': 10,
    ...
    print("\n--- PHASE 2: Full Fine-tuning (Near View v3.0) ---")
    trainer2 = MCFTrainer(overrides=phase2_config, mcf_model=model2.model)
    trainer2.train()
    
    print("\n" + "=" * 70)
    print("✓ Near View MCF v3.0 Training Complete!")
    print("=" * 70)
