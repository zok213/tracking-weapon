...
from ultralytics.nn.modules import (
    ...
    OBB,
    PSA, CrossAttentionShared, CrossMLCA, TensorSelector, CrossMLCAv2, GatedSpatialFusion, GatedSpatialFusion_V3, DeepDiverseBranchBlock, CBAM,
...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        Returns:
            (torch.Tensor): The last output of the model.
        """
        input_img = x # Capture original input (RGB+IR)
        ...
        for m in self.model:
            ...
            # FIX #2: Pass RGB image to GatedSpatialFusion for Illumination Modulation
            if isinstance(m, (GatedSpatialFusion, GatedSpatialFusion_V3)):
                # input_img is (B, 6, H, W). RGB is [:3].
                x = m(x, rgb_image=input_img[:, :3])
            else:
                x = m(x)  # run
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    print(f"DEBUG: YAML loaded dictionary head: {d.get('head', 'MISSING')[:1]}") # Truncate head
    print(f"DEBUG: YAML loaded dictionary backbone tail: {d.get('backbone', 'MISSING')[-5:]}")
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m in (GatedSpatialFusion, GatedSpatialFusion_V3):
            print(f"DEBUG: Parsing GatedSpatialFusion {m} with f={f}")
