#!/usr/bin/env python3
"""
Fine Alignment Sweep Video - Frame 381
Center: (15, 61)
Range: +/- 3 pixels
...
def main():
    ...
    output_path = Path("/home/student/Toan/results/fine_sweep_frame381.mp4")
    ...
    writer = cv2.VideoWriter(str(output_path), cv2.VideoWriter_fourcc(*'mp4v'), 
                            30, (w, h))
    print(f"Generating Fine Sweep Frame 381: {output_path}")
    ...
    for _ in range(15): 
        ...
        overlay[:,:,2] = ir_3ch[:,:,2]
        cv2.putText(overlay, f"({cx:.2f}, {cy:.2f})", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
        cv2.putText(overlay, "Frame 381 | Scale 0.40", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
    ...
    for dx, dy in trajectory:
        ...
        cv2.putText(overlay, f"Frame 381 | Scale 0.40", (10, 60), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
