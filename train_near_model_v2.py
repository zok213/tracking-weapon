#!/usr/bin/env python3
"""
Train Near View MCF Model v2.0 (YOLOv11x RGBT)
ENHANCED with AI Engineer recommendations:
- Early stopping with patience=15
- Label smoothing=0.1
- Better training monitoring with plots
- Save checkpoints every 5 epochs

Optimized for close-range person detection (LasHeR, photo, qiuxing, RGBT234 sources)
"""
import os
import torch
...
def train():
    ...
    print(f"Training NEAR VIEW model v2.0 (ENHANCED - close-range person detection)")
    print(f"Dataset: {dataset_yaml}")
    print(f"Enhancements: early stopping, label smoothing, better monitoring")
    ...
    phase1_last = runs_dir / "near_view_mcf_phase1_v2/weights/last.pt"
    ...
    phase1_config = {
        ...
        'name': 'near_view_mcf_phase1_v2',
        'exist_ok': True,
        'cache': 'ram',
        'workers': 12,
        # NEW: Early stopping and saving
        'patience': 15,
        'save_period': 5,
    }
    
    print("\n--- PHASE 1: Warm-up (Near View v2.0) ---")
    trainer1 = MCFTrainer(overrides=phase1_config, mcf_model=model.model)
    trainer1.train()
    
    # Phase 2: Full Fine-Tuning with ENHANCED settings
    best_phase1 = runs_dir / "near_view_mcf_phase1_v2/weights/best.pt"
    if not best_phase1.exists():
        best_phase1 = runs_dir / "near_view_mcf_phase1_v2/weights/last.pt"
    ...
    phase2_config = {
        ...
        # === AUGMENTATIONS (moderate for near view) ===
        'mosaic': 1.0,
        'mixup': 0.15,
        'copy_paste': 0.1,
        'scale': 0.7,  # Near view: moderate scale augmentation
        'close_mosaic': 10,
        ...
        'freeze': None,
        'project': str(runs_dir),
        'name': 'near_view_mcf_phase2_v2',
        'exist_ok': True,
        'cache': 'ram',
        'workers': 12,
    }
    
    print("\n--- PHASE 2: Full Fine-tuning (Near View v2.0 ENHANCED) ---")
    print("Enhancements applied:")
    print("  - Label smoothing: 0.1")
    print("  - Early stopping patience: 15 epochs")
    print("  - Checkpoint saving: every 5 epochs")
    print("  - Training plots: enabled")
    
    trainer2 = MCFTrainer(overrides=phase2_config, mcf_model=model2.model)
    trainer2.train()
    
    print("\n✓ Near View MCF v2.0 (ENHANCED) Training Complete!")
