#!/usr/bin/env python3
"""
Pretrained Model Benchmark - Scale 0.415
Models:
1. LLVIP-yolo11x-RGBT-midfusion-MCF
2. M3FD-yolo11x-RGBT-midfusion-MCF
"""
import sys
import cv2
import torch
...
OPTIMAL_SCALE = 0.415
X_OFFSET = 0
Y_OFFSET = -105 # Derived from (0, 60) in 512p output
...
def run_inference(nn_model, rgb, ir, conf=0.25, device='cuda:0'):
    h, w = rgb.shape[:2]
    imgsz = 640
    ...
    boxes = []
    if len(det) > 0:
        det[:, :4] *= (w / imgsz) # Scale to original
        boxes = det.cpu().numpy()
...
def process_video(rgb_path, ir_path, out_path, model, title, max_frames=300):
    ...
    while frames_processed < max_frames:
        ...
        boxes = run_inference(model, rgb_a, ir_a, conf=0.25)
...
def main():
    ...
    out_dir = base_dir / "results/pretrained_bench_0415"
    ...
    videos = ["VID_EO_34.mp4"] # Test on main video first
    
    device = 'cuda:0'
    
    print("-" * 60)
    print("Pretrained Benchmark (Scale 0.415)")
    print("-" * 60)
    ...
    for name, path in models:
        ...
        for v_rgb in videos:
            ...
            out_name = f"det_{v_rgb.replace('.mp4','')}_{name}.mp4"
            
            process_video(video_dir/v_rgb, video_dir/v_ir, out_dir/out_name, 
                         nn_model, f"{name} on {v_rgb}")
